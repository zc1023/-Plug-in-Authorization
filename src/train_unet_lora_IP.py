...
data_dir = "/home/czhou/Cplug-in/data/IP/"
print(sys.argv)
...
# sa_lr = float(sys.argv[1])
# ca_lr = float(sys.argv[2])
# rank = int(sys.argv[3])
...
class ImgCaptionSet(torch.utils.data.Dataset):
    ...
    def __getitem__(self,index):
        ...
        try:
            ...
            caption = f'R2D2, high definition, high quality'
        return img,caption
...
def create_data(diffuser,data_path,seed=-1,type = "positive",batch_size = 2):
    ...
    with torch.no_grad():
        ...
        for iteration in range(sample_images//batch_size):
            if type == "positive":    
                images1 = diffuser(prompt = f"A Robot, high definition, high quality",
            ...
            if type == "negative":
                images1 = diffuser(prompt = f"R2D2, high definition, high quality",
                                    generator = generator,   
                                    num_inference_steps=50, 
                                    guidance_scale=7.5,
                                    height=resolution,width=resolution,
...
def train_lora(type):
    ...
    data_path = data_dir+f'/extract-concept/{IP.replace(" ","_")}_{type}'
    output_root_dir=data_dir+f'Lora_model_for_merge/{os.path.basename(data_path)}/lr_{sa_lr}_{ca_lr}_rank_{rank}_epochs_{epochs}'
    ...
    if type == "negative":
        ...
        dir=data_dir+f'/Lora_model_for_merge/{IP.replace(" ","_")}_negative/lr_{sa_lr}_{ca_lr}_rank_{rank}_epochs_{epochs}/{epochs}'
